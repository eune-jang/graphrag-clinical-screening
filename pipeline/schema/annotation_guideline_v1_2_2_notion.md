# Stage 1 Annotation Guideline v1.2.2 (판정 동결본 + 규칙 ID)
...
| v1.2 → v1.2.1 | **판정(adjudication) 착수 전 동결본** (2026-08-25). 기존에 실제로 적용해 온 "종류가 다른 항목 혼재 → split" 규칙이 검토 기록에만 있고 본문에 없던 것을 §3으로 편입 (신규 규칙 아님, 문서화 누락 보정) |

| v1.2.1 → v1.2.2 | **판정 착수 전** 규칙 ID 부여 (2026-08-26). 각 규칙 앞에 `§1-1` 형태의 고유 ID를 붙여 판정 기록의 `rule_id`가 규칙 단위로 특정되게 함. **규칙 내용 변경 없음** — ID 부여와 부록 3(인덱스) 신설뿐 |

**동결 원칙**: 판정이 끝날 때까지 이 문서는 수정하지 않으며, 판정 중 발견되는 개정 필요 사항은 판정 기록(`rule_status=conflict/gap`)으로 남겨 v1.3 재료로 넘긴다.

**규칙 ID 표기법** (v1.2.2 신설)

| 접두 | 영역 | 범위 |
| --- | --- | --- |
| `§1` ~ `§7` | splitting_decision 선택 | §1-1 ~ §7-4 |
| `§T` | Text_span 작성 원칙 | §T0-1 ~ §T6-3 (T1~T6은 기존 (1)~(6)에 대응) |
| `§C` | Cohort_scope 선택 | §C1-1 ~ §C3-4 |

- 판정 기록의 `rule_id`에는 **ID만** 적음 (`§3-4`). 꼬리표를 덧붙이지 않음 — 표기가 갈리면 집계가 깨짐
- ID가 붙지 않은 문장은 규칙이 아니라 **근거·예시·설명**이므로 `rule_id`로 인용하지 않음
- 스펙(온톨로지) 근거로 판정한 경우는 이 문서 소관이 아니므로 `spec §semantic_category`처럼 `spec` 접두사를 사용
- 전체 목록은 **부록 3** 참조
...
- **§1-1** 독립 복수 판정(CRC가 나눠서 생각)이 필요하고, **Umbrella header 없이** 항목들이 결합되어 있을 때 적용.
- child_logic은 원문 표현 대신 *실제 의미*로 판정 (→ child_logic 섹션 참고).

**macro_aggregate**:

- **§1-2** 독립 복수 판정이 필요하고, **Umbrella header 아래** 항목들이 나열되어 있을 때 적용.
    - 예: "adequate organ function" header 아래 lab values, "uncontrolled illness" header 아래 comorbidity 나열.

**nested_exception:**

- **§1-3** inclusion/exclusion criteria 구분없이 구조적 패턴으로 판단
- **§1-4** 본문에서 떨어져 나오는 조건부 예외 구조(트리거)가 있으면 nested_exception
...
- **§1-5** 🔁 **v1.2**: 예외 조각(exception span)은 annotation JSON에 기록하되, Neo4j에서는 별도 child Criterion 노드를 생성하지 않고 단일 Criterion 노드에 INCLUDES_EXCEPTION relation으로 표현됨. composite/macro의 child와 구분하기 위해 "예외 조각"으로 부름. (따라서 예외 조각은 항목별로 더 쪼개지 않으며, child_logic도 부여하지 않음)

---

### 2) splitting_decision 판단 순서

**§2-1** 복합 문장은 다음 **순서**로 판단한다 (순서 고정).
...
- **§2-2** 문장 전체의 큰 덩어리를 먼저 나눈 뒤, 각 덩어리 안에서 포함항목/예외항목을 고려
...
- **§2-3** 한 개의 기준에 nested_exception과 composite_split이 혼재하면 큰 파트를 우선적으로 고려.
...
- **§2-4** 한 번의 판정은 **한 계층**만 다룬다. 화면(또는 프롬프트)에서 하위 계층까지 한꺼번에 입력하지 않음.
- **§2-5** split으로 생긴 조각은 **예외 없이 전부** 다시 판정 대상이 된다. "더 나눠질 것 같은 조각만" 골라 재판정하지 않음(재량 방지). 대부분의 조각은 none으로 즉시 종료됨.
- **§2-6** nested_exception의 경우 **main 조각만** 재판정 대상. 예외 조각은 child가 아니므로 재판정하지 않음.
- **§2-7** 재귀 판정 시 판단 근거는 항상 **루트 criterion 원문 전체**를 보고 정함 (조각만 떼어놓고 판단하지 않음).

---

### 3) split 여부 — 나눌 것인가 말 것인가 (split vs none)

- **§3-1 (query-unit)** CRC가 항목마다 따로 판단하는가(다른 검사·다른 기록·다른 조회 단위)
    - 단일 판정(한 번 판단) → none
...
- **§3-2** 🔁 **v1.2.1 — 종류가 다른 항목이 섞여 있으면 무조건 split**: 한 기준 안에 성격이 명백히 다른 항목(진단 / 약물 / 검사 / 수술·처치 등 = semantic_category가 다른 항목)이 함께 있으면, query-unit을 따질 필요 없이 split.
    ...
    - **§3-3** ※ 적용 범위: 이 규칙은 **나눌지 말지(§3)**에만 쓴다. 어떤 타입으로 나눌지(§4, macro vs composite)를 정할 때는 category를 보지 않는다.

- **§3-4** 🔁 **v1.2 — 외부 문서 위임 → none**: umbrella 표현이 있어도 하위 항목이 프로토콜 텍스트(본문 또는 인접 criterion)에 열거되지 않고 외부 문서(IB, Table, appendix)에만 위임된 경우, 독립 판정 단위가 텍스트에 없으므로 단일 판정 → none. notes에 "thresholds delegated to {외부 문서}" 기록.
    ...
    - **§3-5** 단, 인접 criterion에 실제 lab 수치("ANC ≥ 1500" 등)가 이어서 나열되면 해당 문장은 macro_aggregate parent로 처리 (수치 항목들이 child).

---

### 4) split 타입 결정 (macro_aggregate vs composite_split)

- **§4-1** **Umbrella header 유무**로 결정. (semantic_category가 같은지 다른지는 **타입 결정에서는** 고려하지 않음 — 나눌지 말지 단계에서는 §3의 혼재 규칙이 먼저 적용됨)
...
- 참고)
    - **§4-2** header란 하위 항목을 포괄하는 상위 명사구로, 그 자체가 독립 조회 단위가 아닌 경우(예: "uncontrolled illness", "adequate organ function")
    - **§4-3** 하위 항목이 단일 카테고리로 한 번에 조회되면 header가 아니라 single judgment → none.
...
- **§4-4** 🔁 **v1.2 — header 리트머스 테스트**: header 후보 문구에 **하위 항목들이 커버하지 않는 pass/fail 판정 내용이 남아 있으면, 그 부분은 header가 아니라 별도 child**로 분리한다. header에 숨은 판정은 그래프에서 노드가 되지 못해 매칭 시 평가되지 않고 유실되기 때문.
...
- **§5-1** split(composite_split / macro_aggregate)을 선택한 경우, **3)에서 식별한 독립 판정 단위 수만큼 child를 생성**.
- **§5-2** 하나의 child text_span에 둘 이상의 독립 판정 항목을 함께 넣지 않음. (한 칸에 여러 항목이 들어가면 분리가 실현되지 않아 none과 동일)
...
- **§5-3** macro_aggregate의 Umbrella header는 부모의 구조적 wrapper이며, child는 항목별로 분리.
- **§5-4** 🔁 **v1.2**: 각 child는 저장 후 다시 판정 대상이 된다 (→ 2)-④ 재귀). 이때 child가 또 split이면 그 하위 조각이 생기고, none이면 leaf로 확정된다.

---

### 6) 진단·병기 criterion (3-way 고정)

- **§6-1** 진단(diagnosis) / 병기(stage) / 절제가능성(resectability) 은 각각 독립 query 단위이므로 별도 child로 분할 (3-way). trial 무관하게 granularity 고정.
...
- **§6-2** 병기 세부등급(Stage IIIB vs IIIC 등) 자체는 Stage 카테고리에 해당되는 세부표현이므로 각각을 분리하지 않고 하나의 덩어리로 취급.
- **§6-3** **biomarker(EGFR, ALK, KRAS, NRG1, T790M 등)는 진단과 semantic_category가 다르므로(biomarker ≠ condition) 반드시 별도 child로 분리**
...
- 후속 단계(Stage 2~5)와의 관계
    ...
    - **§6-4** 단, stage · resectability는 Stage 2에서 재결합될 수 있음
    ...
    - **§6-5** 반면 biomarker · 진단은 아예 다른 Concept subtype이라 재결합되지 않음 → 끊기의 필요성은 biomarker·진단 쪽이 더 강함.
...
- **§7-1** 🔁 **v1.2**: composite_split·macro_aggregate **모두** child_logic을 **항상 명시**. (기본값 생략 규칙 폐지 — 생략 시 omit/default 의도 구분 불가. macro_aggregate도 "at least one of the following" 같은 OR 구조가 가능하므로 AND 고정으로 두지 않음)
- **§7-2** nested_exception·none에는 child_logic을 부여하지 않음.
- **§7-3** 선정/제외기준 모두 **표면 표현형("and"/"or") 대신, 실제 의미**로 판정
...
- **§7-4** 🔁 **v1.2 — 혼합 로직은 계층으로 분할**: A AND (B1 OR B2) 형태는 한 parent에 담지 않는다. 계층을 나누고 **각 parent가 자기 child_logic을 하나씩** 가진다.
...
**§T0-1 (대원칙)**: 모든 text_span은 원문의 **연속된 부분 문자열**이어야 함. 오타·대소문자·구두점까지 원문 글자 그대로 잘라내며, 원문에서 검색(Ctrl+F)해서 나오지 않는 문자열은 무효. 판단 기준과 검증 방법이 동일함 — annotator는 검색으로, validator는 substring 검사로 같은 것을 확인.

**§T0-2 (형식)**: text_span은 **항상 세그먼트 배열**로 기록 (대부분 세그먼트 1개). 각 세그먼트는 개별적으로 대원칙(연속 substring)을 충족해야 하며, 기준은 항상 **루트 criterion 원문**.
...
- **§T1-1** 여러 child가 공유하는 개체 표현은 각 child 조각에 끌어다 붙이지 않고 parent 원문에 남겨둠.
...
- **§T1-2** 공통 요구사항이 여러 갈래에 걸리는 경우에도 복제하지 않고, **상위 계층의 child_logic(AND)로 표현**한다 (→ 7) KEYNOTE-671 I3 예시). "공통이니까 각 갈래에 넣는다"는 v1.1 방식은 폐지.

### (2) 같은 판정 단위의 떨어진 조각은 이어붙이지 말고 세그먼트로 나눔

- **§T2-1** 원문 어순상 떨어져 있는 표현들이 하나의 판정 단위에 속하면(예: 병기 관련 표현), 임의로 이어붙여 한 문자열로 만들지 않고 배열에 세그먼트로 나눠 담음.
...
- **§T3-1** "during the treatment period" 같은 시점 표현이나 "if blocks are not available" 같은 조건 표현은 개체가 아니라 **제약**임. 제약은 그 자체로 조회 단위가 될 수 없으므로 자기가 수식하는 child를 따라감 (다음 단계에서 시간·조건 구조화의 입력이 됨).
- **§T3-2** 제약이 여러 child에 걸리면 해당 child 모두의 배열에 동일 세그먼트를 추가함. (각 세그먼트는 연속 substring이므로 대원칙 위반 아님)
...
- **§T3-3** 제약이 일부 child에만 걸리는지 전체에 걸리는지는 표면 위치가 아니라 임상적 의미로 판정 (child_logic 판정과 같은 성격).
- **§T3-4** 하위 계층에서는 상위 조각이 이미 제약을 담고 있으면 다시 붙이지 않음 (상속).
- **§T3-5 (판별 테스트)**: "이 표현 하나만 놓고 CRC가 조회할 수 있는가?" — 조회 불가(제약) → 해당 child마다 세그먼트 추가 / 조회 가능(개체) → 복제 금지, (1) 적용.

### (4) 조각이 그 자체로 완결돼 보이지 않아도 괜찮음

- **§T4-1** "previously untreated"나 "pathologically confirmed"만 떼면 무엇에 대한 것인지 사라지지만 괜찮음. "무엇에 대한"인지는 다음 단계에서 disease concept node와 연결하며 채워짐. stage 1의 목적은 정확히 잘라내는 것까지이며 조각을 완성시키는 게 아님.
    - 예 (KEYNOTE-671 I1): 조각 b = `["pathologically confirmed"]` (무엇을 confirm했는지 안 보여도 OK)
- **§T4-2 (전제)**: 후속 단계(Stage 2~5)가 child 조각과 함께 parent 원문을 입력으로 받아야 함 (파이프라인 prompt에 PARENT_TEXT 필드 포함). 미완결 조각 허용은 이 전제 위에서만 성립.

### (5) nested_exception의 예외 조각(exception span)은 트리거 표현부터

- **§T5-1** 예외 조각의 세그먼트는 예외 트리거 표현의 첫 단어부터 해당 절이 끝나는 지점까지 잘라냄.
...
- **§T5-2** 예외 조각은 항목별로 더 쪼개지 않음 (child가 아니므로 5) child 구성 규칙 미적용).

### (6) 스팬 경계 규칙

- **§T6-1** 시작: 리스트 마커("1)", "a.", "-")와 선행 구두점·공백은 제외하고 첫 단어부터.
- **§T6-2** 끝: 마지막 단어까지 포함하고 후행 쉼표·세미콜론·마침표는 제외.
- **§T6-3** 구두점·괄호가 스팬 중간에 있으면 원문 그대로 포함.
...
- **§C1-1** 기준 텍스트 자체가 적용 범위를 특정 cohort/part/arm으로 제한하는 경우에만 기록.
- 두 가지 시나리오:

**(1) Basket/multi-cohort trial**

- **§C1-2** 서로 다른 tumor type / disease population이 별도 cohort로 분리되며, 해당 선정제외 기준 전체 혹은 일부가 특정 cohort에만 적용되는 경우
...
- **§C1-3** 동일 질환이라도 arm/part 간 서로 다른 eligibility 조건이 텍스트에 명시된 경우
    - 예: "In Part F1, participants with Stage IV NSCLC without prior systemic therapy may be eligible" → `cohort_scope: ["F1"]`
- **§C1-4 (적용하지 않는 경우)**: 단순 drug arm vs placebo arm은 동일 eligibility 공유. metadata에서 추론하지 않으므로 cohort_scope를 기록하지 않음(생략).

### 2) granularity: cohort_scope는 분할 근거가 아님

- **§C2-1** **split 여부는 query-unit으로 정하고, cohort_scope는 그 위에 부여하는 속성**
- **§C2-2** 같은 cohort에 함께 적용되는 조건들을 **cohort가 다르다는 이유로 추가 분할하지 않음.** cohort 그룹(Part) 단위로 묶고, 각 child에 해당되는 cohort_scope를 모두 부여
...
- **§C3-1** Cohort identifier는 프로토콜 원문 표기를 따름 (예: `"A"`, `"F-1"`, `"H"`). 하이픈 유무 등은 해당 trial 원문 그대로.
- **§C3-2** Array 형태로 기록 (예: `["B", "D"]`)
- **§C3-3** `composite_split`에서는 각 child Criterion 노드에 **개별 cohort_scope**를 부여 (parent가 아닌 child 레벨)
- **§C3-4** 🔁 **v1.2**: 모든 cohort에 적용되는 경우 cohort_scope를 기록하지 않음(생략). null·빈 array 표기는 사용하지 않음.
...
| 차이 영역 | 반영 섹션 | rule_id |
| --- | --- | --- |
| macro child 구성 | 5) split 후 child 구성 규칙 | §5-1 ~ §5-3 |
| nested_exception 처리 | 1) 정의 (트리거 목록) + Text_span (5) | §1-3 ~ §1-5, §T5-1, §T5-2 |
| 진단·병기 split 여부 | 6) 진단·병기 입도 (3-way) | §6-1 ~ §6-3 |
| macro/composite 기준 | 4) split 타입 결정 (header 유무 + 리트머스 테스트) | §4-1 ~ §4-4 |
| child_logic 표기 | 7) child_logic 표기 | §7-1 ~ §7-3 |
| 상위 or vs 내부 including | 2) 판단 순서 | §2-1 ~ §2-3 |
| semantic_category에 따른 split | 3) split 여부 (근거 한정) | §3-2, §3-3 |
| cohort_scope 분할 입도 | Cohort_scope 2) 분할 입도 | §C2-1, §C2-2 |
| (v1.2 신설) 혼합 로직·재귀 분할 | 2) 판단 순서 ④ + 7) 혼합 로직 예시 | §2-4 ~ §2-7, §7-4 |
| (v1.2 신설) 외부 문서 위임 | 3) split 여부 | §3-4, §3-5 |
| (v1.2 신설) 스팬 경계·세그먼트 배열 | Text_span 대원칙·(2)·(6) | §T0-1, §T0-2, §T2-1, §T6-1 ~ §T6-3 |
...
판정 기록에 인용할 수 있는 규칙 전체 목록. 여기 없는 문장은 규칙이 아님.

### §1 라벨 정의

| rule_id | 규칙 |
| --- | --- |
| §1-1 | composite_split — 독립 복수 판정 + umbrella header **없음** |
| §1-2 | macro_aggregate — 독립 복수 판정 + umbrella header **아래** 나열 |
| §1-3 | nested_exception — inclusion/exclusion 무관, 구조적 패턴으로 판단 |
| §1-4 | 예외 트리거 표현 목록 (`except`, `unless`, `not required if ~` 등) |
| §1-5 | 예외 조각은 child 아님 — 더 쪼개지 않고 child_logic도 없음 |

### §2 판단 순서·재귀

| rule_id | 규칙 |
| --- | --- |
| §2-1 | 판단 순서 고정 (① 최상위 결합 → ② 예외 → ③ split 여부·타입 → ④ 재귀) |
| §2-2 | 최상위 결합을 먼저 나누고, 파트 내부의 "including ~"은 나누지 않음 |
| §2-3 | nested_exception vs composite_split — B 전체가 예외면 nested, B 일부면 composite |
| §2-4 | 한 번의 판정은 한 계층만 |
| §2-5 | split으로 생긴 조각은 예외 없이 전부 재판정 |
| §2-6 | nested_exception은 main 조각만 재판정 |
| §2-7 | 재귀 판정도 근거는 루트 criterion 원문 전체 |

### §3 split 여부 (split vs none)

| rule_id | 규칙 |
| --- | --- |
| §3-1 | query-unit — 단일 판정 → none / 독립 복수 판정 → split |
| §3-2 | semantic_category가 다른 항목 혼재 → query-unit 불문 split |
| §3-3 | §3-2의 적용 범위 — 나눌지 말지에만 사용, 타입 결정에는 미사용 |
| §3-4 | 외부 문서 위임(IB·Table·appendix)만 있으면 → none + notes 기록 |
| §3-5 | 인접 criterion에 실제 수치가 나열되면 macro_aggregate parent |

### §4 split 타입 (macro vs composite)

| rule_id | 규칙 |
| --- | --- |
| §4-1 | umbrella header 유무로 타입 결정 |
| §4-2 | header 정의 — 하위 항목을 포괄하는 상위 명사구, 그 자체가 독립 조회 단위 아님 |
| §4-3 | 하위 항목이 단일 카테고리로 한 번에 조회되면 header 아님 → none |
| §4-4 | header 리트머스 테스트 — header에 남은 pass/fail은 별도 child |

### §5 child 구성

| rule_id | 규칙 |
| --- | --- |
| §5-1 | 독립 판정 단위 수만큼 child 생성 |
| §5-2 | 한 child text_span에 둘 이상의 판정 항목 금지 |
| §5-3 | macro의 umbrella header는 parent의 구조적 wrapper |
| §5-4 | 각 child는 저장 후 재판정 대상 (→ §2-1 ④) |

### §6 진단·병기 criterion

| rule_id | 규칙 |
| --- | --- |
| §6-1 | 진단 / 병기 / 절제가능성 3-way 고정 (trial 무관) |
| §6-2 | 병기 세부등급은 하나의 덩어리로 취급 |
| §6-3 | biomarker는 진단과 semantic_category가 달라 반드시 별도 child |
| §6-4 | stage · resectability는 Stage 2에서 재결합될 수 있음 |
| §6-5 | biomarker · 진단은 Concept subtype이 달라 재결합되지 않음 |

### §7 child_logic

| rule_id | 규칙 |
| --- | --- |
| §7-1 | composite_split·macro_aggregate 모두 child_logic 항상 명시 |
| §7-2 | nested_exception·none에는 child_logic 미부여 |
| §7-3 | 표면 접속사가 아니라 실제 의미로 AND/OR 판정 |
| §7-4 | 혼합 로직(A AND (B1 OR B2))은 계층으로 분할 |

### §T Text_span

| rule_id | 규칙 |
| --- | --- |
| §T0-1 | 대원칙 — 원문의 연속 부분 문자열 (글자 그대로) |
| §T0-2 | 형식 — 항상 세그먼트 배열, 기준은 루트 criterion 원문 |
| §T1-1 | 개체성 공통 표현(질환명·약물명)은 child에 복제하지 않음 |
| §T1-2 | 공통 요구사항은 복제 대신 상위 계층 child_logic(AND)로 표현 |
| §T2-1 | 같은 판정 단위의 떨어진 조각은 이어붙이지 말고 세그먼트로 |
| §T3-1 | 시점·수치·조건 제약 표현은 수식하는 child를 따라감 |
| §T3-2 | 제약이 여러 child에 걸리면 각 child 배열에 동일 세그먼트 추가 |
| §T3-3 | 제약의 적용 범위는 표면 위치가 아니라 임상적 의미로 판정 |
| §T3-4 | 상위 조각이 이미 제약을 담고 있으면 하위에서 재부착하지 않음 (상속) |
| §T3-5 | 판별 테스트 — 이 표현만으로 CRC가 조회 가능한가 (개체 vs 제약) |
| §T4-1 | 조각이 그 자체로 완결돼 보이지 않아도 됨 |
| §T4-2 | §T4-1의 전제 — 후속 단계가 PARENT_TEXT를 함께 입력받아야 성립 |
| §T5-1 | 예외 조각은 트리거 표현 첫 단어부터 절 끝까지 |
| §T5-2 | 예외 조각은 항목별로 더 쪼개지 않음 |
| §T6-1 | 시작 경계 — 리스트 마커·선행 구두점 제외 |
| §T6-2 | 끝 경계 — 후행 쉼표·세미콜론·마침표 제외 |
| §T6-3 | 스팬 중간의 구두점·괄호는 원문 그대로 포함 |

### §C Cohort_scope

| rule_id | 규칙 |
| --- | --- |
| §C1-1 | 기준 텍스트 자체가 적용 범위를 제한할 때만 기록 |
| §C1-2 | Basket/multi-cohort trial 시나리오 |
| §C1-3 | 단일 질환 trial 내 arm/part별 조건 분화 시나리오 |
| §C1-4 | 단순 drug arm vs placebo arm은 미적용(생략) |
| §C2-1 | split 여부는 query-unit으로 정하고 cohort_scope는 그 위의 속성 |
| §C2-2 | cohort가 다르다는 이유로 추가 분할하지 않음 |
| §C3-1 | cohort identifier는 프로토콜 원문 표기 그대로 |
| §C3-2 | array 형태로 기록 |
| §C3-3 | composite_split은 child 레벨에 개별 부여 |
| §C3-4 | 전 cohort 적용 시 생략 (null·빈 array 미사용) |
